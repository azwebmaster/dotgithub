...
/** Output parameters for the Checkout action */
...
/**
 * Checkout a Git repository at a particular version - Step Chain Version
 *
 * https://github.com/actions/checkout/tree/v4
 * @param inputs Optional input parameters
 * @param step Optional step configuration overrides
 * @param ref Optional git reference (defaults to the version used for generation)
 * @returns A StepChainBuilder for chaining steps
 */
export function checkoutChain(
  this: ActionCollection,
  inputs?: CheckoutInputs,
  step?: Partial<Omit<GitHubStepAction, "uses" | "with">>,
  ref?: string,
): StepChainBuilder<CheckoutOutputs> {
  ...
  return this.stepChain(
    actionStep,
  ) as unknown as StepChainBuilder<CheckoutOutputs>;
}
