  ...
  return new Command('list')
    ...
    .action(async (options) => {
      try {
        ...
        logger.info(
          `Found ${actions.length} tracked action${actions.length === 1 ? '' : 's'}:`
        );

        actions.forEach((action) => {
          const actionIdentifier = action.actionPath
            ? `${action.orgRepo}/${action.actionPath}`
            : action.orgRepo;
          logger.info(`  📦 ${actionIdentifier}`);
          logger.debug(
            `     Function Name: ${action.actionName ? generateFunctionName(action.actionName) : action.orgRepo}()`
          );
          logger.debug(`     Version: ${action.versionRef}`);
          logger.debug(`     SHA: ${action.ref}`);
          logger.debug(
            `     Output Path: ${action.outputPath ? context.resolvePath(action.outputPath) : 'Not generated'}`
          );
        });
      } catch (err) {
