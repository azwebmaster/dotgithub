...
export function createConfigCommand(
  ...
  configCmd
    ...
    .action((options) => {
      try {
        ...
        if (actions.length === 0) {
          logger.info(
            'No actions tracked yet. Add actions with "dotgithub add <action>"'
          );
          return;
        }
        ...
        actions.forEach((action) => {
          ...
          logger.info(`\n• ${functionName}()`);
          logger.info(
            `  Repository: ${action.orgRepo}@${action.versionRef} (SHA: ${action.ref.substring(0, 8)})`
          );
          logger.info(
            `  Output: ${action.outputPath ? context.resolvePath(action.outputPath) : 'Not generated'}`
          );
        });
      } catch (err) {
        logger.error('Configuration error', {
  ...
  configCmd
    ...
    .option(
      '--output-dir <dir>',
      'Output directory for actions',
      '.github/actions'
    )
    .option(
      '--format <format>',
      'Config file format (json, js, yaml, yml)',
      'json'
    )
    .action((options) => {
      try {
        ...
        if (!['json', 'js', 'yaml', 'yml'].includes(format)) {
          logger.error(
            'Invalid format. Supported formats: json, js, yaml, yml'
          );
          process.exit(1);
        }
        ...
        if (fs.existsSync(context.configPath)) {
          logger.info(
            `Configuration file already exists at: ${context.configPath}`
          );
          logger.info(
            'Use "dotgithub config show" to view current configuration'
          );
          return;
        }
