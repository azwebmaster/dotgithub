...
export function createUpdateCommand(
  ...
  return new Command('update')
    .argument(
      '[orgRepoRef]',
      'GitHub repository reference (e.g., actions/checkout). If not provided, updates all actions'
    )
    ...
    .option(
      '--latest',
      'Use the latest git tag with semver parsing instead of versionRef'
    )
    ...
    .action(async (orgRepoRef, options) => {
      try {
        ...
        if (result.updated.length > 0) {
          logger.success(
            `Successfully updated ${result.updated.length} action(s):`
          );
          for (const update of result.updated) {
            logger.info(
              `  ${update.orgRepo}: ${update.previousVersion} → ${update.newVersion}`
            );
