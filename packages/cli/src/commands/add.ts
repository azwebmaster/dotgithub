...
import { generateActionFiles, type DotGithubContext, logger } from '@dotgithub/core';
      ...
      try {
        ...
        for (const orgRepoRef of orgRepoRefs) {
          ...
          if (result.actions && result.actions.length > 0) {
            logger.debug(`Generated ${result.actions.length} action(s) from ${orgRepoRef}`);
            for (const action of result.actions) {
              const location = action.actionPath ? `${action.actionPath}/${action.actionName}` : action.actionName;
              logger.debug(`Generated action: ${location}`, { filePath: action.filePath });
              totalActions.push({ orgRepoRef, ...action });
            }
          } else {
            logger.debug(`Generated action: ${result.actionName}`, { filePath: result.filePath });
        ...
        if (totalActions.length > 1 || orgRepoRefs.length > 1) {
          logger.success(`Successfully generated ${totalActions.length} action(s) from ${orgRepoRefs.length} repository(ies)`);
        } else {
          logger.success(`Successfully generated ${totalActions[0]?.actionName || 'action'}`);
        }
