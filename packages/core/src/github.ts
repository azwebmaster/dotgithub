  ...
  try {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Could not resolve ref "${ref}" to a SHA for ${owner}/${repo}`
        );
      }
    }
  }
...
export async function getLatestTag(
  ...
  try {
    ...
    if (sortedMajorTags.length > 0) {
      ...
      if (!latestMajorTag) {
        throw new Error(
          `Unexpected error: no major version tag found despite having ${sortedMajorTags.length} tags`
        );
      }
    ...
    throw new Error(
      `Failed to get latest tag for ${owner}/${repo}: ${error instanceof Error ? error.message : error}`
    );
  }
}

export async function getLatestTagSafe(
  owner: string,
  repo: string,
