...
describe('config', () => {
  ...
  describe('readConfig and writeConfig', () => {
    ...
    it('creates .github directory if it does not exist', () => {
      ...
      fs.rmSync(path.join(tempDir, '.github'), {
        recursive: true,
        force: true,
      });
      ...
      expect(
        fs.existsSync(path.join(tempDir, '.github', 'dotgithub.json'))
      ).toBe(true);
    });
  });
  ...
  describe('addActionToConfig', () => {
    it('adds new action to empty config', () => {
      ...
      expect(path.basename(resolvedPath)).toBe(
        path.basename(absoluteOutputPath)
      );
      expect(path.basename(path.dirname(resolvedPath))).toBe(
        path.basename(path.dirname(absoluteOutputPath))
      );
    });
    ...
    it('updates existing action with same orgRepo and ref', () => {
      ...
      const updatedOutputPath = path.join(
        outputDir,
        'actions',
        'checkout',
        'checkout.ts'
      ...
      const resolvedPath = getResolvedOutputPath(actions[0]!);
      expect(path.basename(resolvedPath)).toBe(
        path.basename(updatedOutputPath)
      );
      expect(path.basename(path.dirname(resolvedPath))).toBe(
        path.basename(path.dirname(updatedOutputPath))
      );
    });
  ...
  describe('removeActionFromConfig', () => {
    ...
    it('removes action by orgRepo', () => {
      ...
      expect(actions).toHaveLength(2);
      expect(
        actions.find((a) => a.orgRepo === 'actions/checkout')
      ).toBeUndefined();
    });
    ...
    it('removes different orgRepo action', () => {
      ...
      expect(actions).toHaveLength(2);
      expect(
        actions.find((a) => a.orgRepo === 'actions/setup-node')
      ).toBeUndefined();
    });
  ...
  describe('action matching by orgRepo and actionPath', () => {
    ...
    it('should match actions by both orgRepo and actionPath', () => {
      ...
      const checkoutActions = actions.filter(
        (a) => a.orgRepo === 'actions/checkout'
      );
  ...
  describe('path resolution functions', () => {
    it('getActionsFromConfigWithResolvedPaths returns resolved absolute paths', () => {
      ...
      expect(actionsWithResolvedPaths[0]!.outputPath).toBe(
  ...
  describe('multiple config formats', () => {
    ...
    describe('createConfigFile', () => {
      ...
      it('throws error if config file already exists', () => {
        ...
        expect(() => createConfigFile('json')).toThrow(
